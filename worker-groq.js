...
// ⚠️ 2026-08-09(413fix-8): 워드프레스 플러그인 코드(fetch_topic_research_from_worker)를
// 실측 재확인한 결과, 실제 wp_remote_post 콜 타임아웃은 "최소 8초"가 아니라
// 1차 시도 약 30초, 설령 1차가 실패해도 2차 시도에 최소 15~20초를 보장하도록
// 되어 있었다(58초 총 예산 중 max(15, min(30, remaining-8)) 공식). 즉 이전 버전의
// "워드프레스가 8초 만에 끊을 수도 있다"는 전제가 지나치게 보수적이었고, 그로
// 인해 FETCH_TIMEOUT_MS(12초)가 groq/compound의 실제 검색 응답 시간보다 짧아
// 정상적으로 진행 중이던 요청을 Worker 스스로 abort시키는 문제가 실사용에서
// 확인되었다(에러: "Groq API 호출 실패(네트워크/타임아웃): The operation was aborted").
// → 워드프레스가 보장하는 최소 시간(15초) 안에서 안전하게 끝나도록, 그러나
// groq/compound의 웹검색 왕복 시간을 충분히 감당하도록 각 단계 타임아웃을 올린다.
...
// 캐시되었거나 재배포가 안 된 것이다.
const WORKER_VERSION = '2026-08-09-413fix-8-timeout-realign';
...
// ⚠️ 2026-08-09(413fix-8): 워드프레스 플러그인 소스 재확인 결과, AI 썸네일
// 조사 흐름(fetch_topic_research_from_worker)은 1차 시도에 최대 30초, 설령
// 1차가 실패해도 2차 시도에 최소 15초를 항상 보장하도록 되어 있다(58초 총
// 예산 기준). 즉 Worker가 실제로 쓸 수 있는 시간은 기존에 가정했던 8~16초가
// 아니라 최소 15초, 통상 30초에 가깝다. 총 예산을 28초로 올려 1차 시도의
// 30초 타임아웃 안에 여유(약 2초)를 두고 안전하게 들어오게 하되, 여전히
// 워드프레스가 절대 먼저 끊지 않도록 30초보다는 항상 작게 유지한다.
